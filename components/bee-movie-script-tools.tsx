...
export default function BeeMovieScriptTools() {
  const [selectedFormat, setSelectedFormat] = useState<
    "normal" | "nospaces" | "segments"
  >("normal");
  const [platformLimit, setPlatformLimit] = useState(280);
  ...
  const renderContentArea = (
    renderContent: (data: ScriptData) => React.ReactNode
  ) => {
    if (isLoading) {
      return (
    ...
    >
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-lg text-muted-foreground max-w-4xl mx-auto">
          Choose from multiple formats: normal script, no-spaces compressed
          version, auto-segmented for platform limits, and toggle display
          options like character names and blank lines.
        </p>
      </div>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center space-x-2">
              <Switch
                id="hide-characters"
                checked={hideCharacters}
                onCheckedChange={setHideCharacters}
              />
            ...
            <p className="text-xs text-muted-foreground">
              When enabled, only dialogue and narration will be shown, character
              names will be removed from the Bee Movie script
            </p>
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={setHideBlankLines}
              />
            ...
            <p className="text-xs text-muted-foreground">
              When enabled, all empty lines will be removed to create a more
              compact text format
            </p>
          </div>
        </CardContent>
            ...
            <Button
              onClick={() =>
                handleCopyAction((data) =>
                  removeSpaces(getOpeningSection(data))
                )
              }
              variant="outline"
              className="w-full h-10"
      ...
      >
              ...
              <div className="space-y-4">
                {scriptData && (
                  <div className="flex gap-2">
                    <Button
                      onClick={() =>
                        copyToClipboard(processText(scriptData.scriptContent))
                      }
                      disabled={isLoading}
                    >
                      <Copy className="mr-2 h-4 w-4" />
        ...
        <TabsContent value="nospaces" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Compressed Bee Movie script version for platforms with character
                limits or spam detection
                {hideCharacters && " (dialogue only)"}
                {hideBlankLines && " (compact view)"}
              </CardDescription>
            </CardHeader>
                ...
                {scriptData && (
                  <div className="flex gap-2">
                    <Button
                      onClick={() =>
                        copyToClipboard(
                          removeSpaces(processText(scriptData.scriptContent))
                        )
                      }
                      disabled={isLoading}
                    >
                    ...
                    <Badge variant="secondary">
                      {removeSpaces(
                        processText(scriptData.scriptContent)
                      ).length.toLocaleString()}{" "}
                      chars
                    </Badge>
                  </div>
        ...
        <TabsContent value="segments" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>
                Auto-Segmented Bee Movie Script for Platforms
              </CardTitle>
              <CardDescription>
                Automatically split the entire Bee Movie script for Discord
                (2000 chars), Twitter, etc.
                {hideCharacters && " (dialogue only)"}
                {hideBlankLines && " (compact view)"}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4 ">
                ...
                <div className="max-h-200 overflow-y-auto space-y-6">
                  {renderContentArea((data) =>
                    segmentText(
                      processText(data.scriptContent),
                      platformLimit
                    ).map((segment, index) => (
                      <div key={index}>
                        {index >= 0 && (
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="rounded-lg bg-muted p-4 font-mono text-sm whitespace-pre-wrap overflow-x-auto break-words">
                            {segment}
                          </div>
                        </div>
                      </div>
      ...
      <Card className="mt-8" id="quotes">
        <CardHeader>
          <CardTitle>
            <h3>Popular Bee Movie Meme Quotes</h3>
          </CardTitle>
          <CardDescription>
            Quick access to the most popular Bee Movie script quotes for memes
            and copy paste
            {hideCharacters && " (dialogue only)"}
            {hideBlankLines && " (compact view)"}
          </CardDescription>
            ...
            {renderContentArea((data) =>
              data.popularQuotes.map((quote, index) => {
                ...
                return (
                  ...
                  >
                    <span className="text-sm font-mono flex-1 mr-3 whitespace-pre-wrap break-words overflow-hidden">
                      {processedQuote}
                    </span>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => copyToClipboard(processedQuote)}
                    >
      ...
      <Card className="mt-16" id="stats">
        <CardHeader>
          <CardTitle>
            <h3>Bee Movie Script Statistics</h3>
          </CardTitle>
          <CardDescription>
            Complete analysis of the entire Bee Movie script content
          </CardDescription>
        </CardHeader>
        <CardContent>
